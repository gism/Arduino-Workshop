...
void loop() {
    ...
    float radianshourNY = ((((now.hour() + 6) * 360) / 12) + 270) * (PI / 180);
    int hourxNY = cos(radianshourNY) * (clocksize / 2.5);
    int houryNY = sin(radianshourNY) * (clocksize / 2.5);
    ...
    int NYcenterX = 3*centerX;
    display.drawCircle(NYcenterX, centerY, clocksize, WHITE);
    
    display.drawLine(NYcenterX,centerY,NYcenterX+hourxNY,centerY+houryNY,WHITE);
    display.drawLine(NYcenterX,centerY,NYcenterX+minx,centerY+miny,WHITE);
    display.drawLine(NYcenterX,centerY,NYcenterX+secx,centerY+secy,WHITE);
